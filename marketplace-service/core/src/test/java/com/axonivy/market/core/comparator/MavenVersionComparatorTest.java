...
class MavenVersionComparatorTest {
  ...
  void testCompare() {
    ...
    assertTrue(result > 0, "Newer version should compare after older version in natural order");

    // Compare: release vs snapshot
    result = comparator.compare("1.0.0", "1.0.0-SNAPSHOT");
    assertTrue(result > 0, "Release should compare after snapshot in natural order");

    // Compare: pre-release vs release
    result = comparator.compare("1.0.0", "1.0.0-alpha");
    assertTrue(result > 0, "Release should compare after pre-release qualifiers in natural order");

    // Sorting behavior with newest version first
    List<String> versions = Arrays.asList("1.0.0", "2.0.0", "1.10.0", "1.2.0");
    List<String> sortedVersions = versions.stream().sorted(comparator.reversed()).toList();
