...
public class ZipSafetyScanner {
        ...
        try (ZipFile zipFile = new ZipFile(tempFile)) {
            ...
            while (entries.hasMoreElements()) {
                ...
                while ((nBytes = in.read(buffer)) > 0) {
                    ...
                    isEntrySizeValid(totalSizeEntry, ze, totalSizeArchive, totalEntryArchive);
                }

                // Check if README.md exists (case-insensitive)
                if ("README.md".equalsIgnoreCase(ze.getName())) {
                    hasReadme = true;
                }
            }
    ...
    private static void isEntrySizeValid(double totalSizeEntry, ZipEntry ze, int totalSizeArchive, int totalEntryArchive) {
