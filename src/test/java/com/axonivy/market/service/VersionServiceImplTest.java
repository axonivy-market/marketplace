...
class VersionServiceImplTest {
	...
	private void setUpArtifactFromMeta() {
		...
		MavenArtifact additionalMavenArtifact = new MavenArtifact(repoUrl, "", groupId, artifactId, "", null, null,
				null);
	...
	void testGetArtifactsAndVersionToDisplay() {
		...
		Assertions.assertEquals(1,
				versionService.getArtifactsAndVersionToDisplay(productId, false, targetVersion).size());
	...
	void testGetVersionsToDisplay() {
		...
		when(versionService.getVersionsFromArtifactDetails(repoUrl, groupId, artifactId))
				.thenReturn(versionFromArtifact);
	...
	void testBuildMavenMetadataUrlFromArtifact() {
		...
		Assertions.assertEquals(StringUtils.EMPTY,
				versionService.buildMavenMetadataUrlFromArtifact(repoUrl, groupId, null), StringUtils.EMPTY);
	...
	void testIsReleasedVersionOrUnReleaseDevVersion() {
		...
		Assertions
				.assertTrue(versionService.isOfficialVersionOrUnReleasedDevVersion(versions, unreleasedSprintVersion));
	}
	...
	void testConvertMavenArtifactsToModels() {
		...
		List<MavenArtifactModel> result = versionService.convertMavenArtifactsToModels(Collections.emptyList(),
				"10.0.21");
	...
	void testBuildDownloadUrlFromArtifactAndVersion() {
		...
		archivedArtifactsMap.put(targetArtifactId,
		...
		expectedResult = String.format(MavenConstants.ARTIFACT_DOWNLOAD_URL_FORMAT, customRepoUrl,
				"com/axonivy/adobe/connector", "adobe-connector", targetVersion, "adobe-connector", targetVersion,
				customType);
		Assertions.assertEquals(expectedResult, result);
	}
