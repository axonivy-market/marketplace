...
describe('SuccessDialogComponent', () => {
  ...
  it('should display translated thank message with displayName', () => {
    ...
    mockAuthService.getDisplayName.mockReturnValue(mockDisplayName);
    fixture.detectChanges();
    const compiled = fixture.nativeElement;
    const thankMessageElement = compiled.querySelector(
      '.modal-body h4.text-primary:nth-child(3)'
    );
    expect(thankMessageElement).toBeTruthy();
    expect(thankMessageElement.textContent.trim()).toContain(
      `common.feedback.thankMessage ${mockDisplayName}`
    );
  });

  it('should dismiss modal when close button is clicked', () => {
    const dismissSpy = vi.spyOn(component.activeModal, 'dismiss');
