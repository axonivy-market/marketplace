...
describe('ProductDetailInformationTabComponent', () => {
  ...
  beforeEach(async () => {
    const productDetailServiceSpy = {
      getExternalDocumentForProductByVersion: vi
        .fn()
        .mockName(
          'ProductDetailService.getExternalDocumentForProductByVersion'
        ),
      getBestMatchVersion: vi
        .fn()
        .mockName('ProductDetailService.getBestMatchVersion')
    };
  ...
  it('should set externalDocumentLink and displayExternalDocName on valid version change', () => {
    productDetailService.getBestMatchVersion.mockReturnValue(of(TEST_VERSION));
    productDetailService.getExternalDocumentForProductByVersion.mockReturnValue(
  ...
  it('should set externalDocumentLink and displayExternalDocName on null response', () => {
    productDetailService.getBestMatchVersion.mockReturnValue(of(TEST_VERSION));
    (
      productDetailService.getExternalDocumentForProductByVersion as Mock
  ...
  it('should set externalDocumentLink and displayExternalDocName on getBestMatch error', () => {
    productDetailService.getBestMatchVersion.mockReturnValue(
      throwError(() => new Error('Network error'))
    );
    component.productDetail = {
      id: TEST_ID,
  ...
  describe('ngOnInit', () => {
    it('should set displayVersion from selectedVersion', () => {
      ...
      vi.spyOn(component, 'extractVersionValue').mockReturnValue(TEST_VERSION);
  ...
  it('should return false when change is undefined', () => {
    expect(component.isVersionUnchangedOrFirstChange(undefined)).toBe(false);
  });

  it('should return true when currentValue equals previousValue', () => {
    const change = new SimpleChange('v1', 'v1', false);
    expect(component.isVersionUnchangedOrFirstChange(change)).toBe(true);
  });
