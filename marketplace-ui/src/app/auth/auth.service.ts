...
export class AuthService {
  ...
  constructor(
    ...
    private readonly cookieService: CookieService,
    private readonly httpBackend: HttpBackend,
    private readonly windowRef: WindowRef // <-- injected
  ) {
    this.httpClientWithoutInterceptor = new HttpClient(httpBackend);

    const win = this.windowRef.nativeWindow;
    this.githubAuthCallbackUrl = (win?.location?.origin ?? '') + environment.githubAuthCallbackPath; // <-- SSR-safe
  }
  ...
  redirectToGitHub(originalUrl: string): void {
    ...
    const win = this.windowRef.nativeWindow; // <-- SSR-safe
    if (win) {
